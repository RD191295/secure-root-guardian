...
export const Chip3DEnvironment: React.FC<Chip3DEnvironmentProps> = ({
  ...
  // Module positions: PMU(5,50,210x180), Crypto(320,50,160x180), OTP(600,50,140x180)
  // Flash(5,400,140x190), CPU(320,320,160x180), ROM(600,400,140x180)
  const traces = useMemo(() => [
    // ===== POWER DISTRIBUTION (Stage 1+) =====
    // PMU right center (215,140) → Crypto left center (320,140)
    { points: [{ x: 215, y: 140 }, { x: 320, y: 140 }], type: 'power' as const, label: 'VCC', active: currentStage >= 1 },
    
    // PMU bottom center (110,230) → Flash top center (75,400)
    { points: [{ x: 110, y: 230 }, { x: 110, y: 315 }, { x: 75, y: 315 }, { x: 75, y: 400 }], type: 'power' as const, label: 'VCC', active: currentStage >= 1 },
    
    // Crypto right center (480,140) → OTP left center (600,140)
    { points: [{ x: 480, y: 140 }, { x: 600, y: 140 }], type: 'power' as const, label: 'VCC', active: currentStage >= 1 },
    
    // Crypto bottom center (400,230) → CPU top center (400,320)
    { points: [{ x: 400, y: 230 }, { x: 400, y: 320 }], type: 'power' as const, label: 'VCC', active: currentStage >= 1 },
    
    // OTP bottom center (670,230) → ROM top center (670,400)
    { points: [{ x: 670, y: 230 }, { x: 670, y: 400 }], type: 'power' as const, label: 'VCC', active: currentStage >= 1 },

    // ===== STAGE 2: ROM → OTP (Key Request) =====
    // ROM top-left (650,400) → OTP bottom-left (650,230)
    { points: [{ x: 650, y: 400 }, { x: 650, y: 230 }], type: 'control' as const, label: 'KEY_REQ', active: currentStage === 2 },
    
    // ===== STAGE 2-3: OTP → ROM (Key Data Response) =====
    // OTP bottom-right (690,230) → ROM top-right (690,400)
    { points: [{ x: 690, y: 230 }, { x: 690, y: 400 }], type: 'data' as const, label: 'KEY_HASH', active: currentStage >= 2 && currentStage <= 3 },

    // ===== STAGE 3: ROM → Flash (Bootloader Read Request) =====
    // ROM left-upper (600,440) → Flash right-middle (145,480)
    { points: [{ x: 600, y: 440 }, { x: 370, y: 440 }, { x: 370, y: 480 }, { x: 145, y: 480 }], type: 'control' as const, label: 'READ_BL', active: currentStage === 3 },
    
    // ===== STAGE 3-4: Flash → ROM (Bootloader Data) =====
    // Flash right-lower (145,510) → ROM left-middle (600,490)
    { points: [{ x: 145, y: 510 }, { x: 350, y: 510 }, { x: 350, y: 490 }, { x: 600, y: 490 }], type: 'data' as const, label: 'BL_DATA', active: currentStage >= 3 && currentStage <= 4 },

    // ===== STAGE 4-5: ROM → Crypto (Public Key) =====
    // ROM left-top (600,420) → Crypto bottom-right (460,230)
    { points: [{ x: 600, y: 420 }, { x: 540, y: 420 }, { x: 540, y: 270 }, { x: 460, y: 270 }, { x: 460, y: 230 }], type: 'data' as const, label: 'PUB_KEY', active: currentStage >= 4 && currentStage <= 5 },
    
    // ===== STAGE 4-5: Flash → Crypto (Signature) =====
    // Flash right-top (145,420) → Crypto left-bottom (320,200)
    { points: [{ x: 145, y: 420 }, { x: 230, y: 420 }, { x: 230, y: 200 }, { x: 320, y: 200 }], type: 'data' as const, label: 'SIG', active: currentStage >= 4 && currentStage <= 5 },
    
    // ===== STAGE 5: Crypto → ROM (Verification Result) =====
    // Crypto right-middle (480,150) → ROM left-upper (600,430)
    { points: [{ x: 480, y: 150 }, { x: 550, y: 150 }, { x: 550, y: 430 }, { x: 600, y: 430 }], type: 'control' as const, label: mode === 'tampered' ? 'FAIL' : 'PASS', active: currentStage === 5 },

    // ===== STAGE 6: ROM → CPU (Control Transfer) =====
    // ROM left-lower (600,500) → CPU right-middle (480,410)
    { points: [{ x: 600, y: 500 }, { x: 540, y: 500 }, { x: 540, y: 410 }, { x: 480, y: 410 }], type: 'control' as const, label: mode === 'tampered' ? 'HALT' : 'BOOT', active: currentStage === 6 },

    // ===== STAGE 7: CPU ↔ Flash (OS/Application Access) =====
    // CPU left-upper (320,370) → Flash right-top (145,430)
    { points: [{ x: 320, y: 370 }, { x: 230, y: 370 }, { x: 230, y: 430 }, { x: 145, y: 430 }], type: 'control' as const, label: 'READ', active: currentStage >= 7 },
    
    // Flash right-middle (145,460) → CPU left-middle (320,400)
    { points: [{ x: 145, y: 460 }, { x: 210, y: 460 }, { x: 210, y: 400 }, { x: 320, y: 400 }], type: 'data' as const, label: mode === 'tampered' ? 'SAFE' : 'OS', active: currentStage >= 7 },
  ], [currentStage, mode]);
