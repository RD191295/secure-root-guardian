...
const PCBSimulation: React.FC = () => {
  const [activeTraces, setActiveTraces] = useState<number[]>([0,1,2,3]); // all active by default
  ...
  return (
    <div className="flex justify-center items-center h-[600px] bg-slate-950">
      <svg width="800" height="500" viewBox="0 0 800 500">
        {/* Pipeline */}
        <rect x="100" y="230" width="600" height="40" rx="20" fill="cyan" fillOpacity={0.3} stroke="cyan" strokeWidth={2}/>
        <text x="400" y="255" className="fill-cyan-300 text-sm font-mono" textAnchor="middle">MAIN DATA PIPELINE</text>
        ...
        {traces.map((trace,index)=>(
          <PCBTrace key={index} from={trace.from} to={trace.to} isActive={true} type={trace.type} label={trace.label} dotCount={5}/>
        ))}
      </svg>
    </div>
  );
